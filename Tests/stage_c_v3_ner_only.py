"""
Stage C v3: NER only – no label filtering.

Unlike v1 and v2, this version does NOT filter entities by label (Medication,
Disease_disorder, etc.). All entity types from the NER model pass through.
Filter by UMLS in Stage D instead.

Usage:
  python Tests/stage_c_v3_ner_only.py
  python Tests/stage_c_v3_ner_only.py --input-dir outputs/STAGE_B_v1
  python Tests/stage_c_v3_ner_only.py --output-dir outputs/STAGE_C_v3
"""
import os
import sys
import json
...
V3_OUTPUT_DIR = config.OUTPUTS_ROOT / "STAGE_C_v3"
...
def run_stage_c_v3(
    ...
    out_root = Path(output_dir) if output_dir else V3_OUTPUT_DIR
    out_root.mkdir(parents=True, exist_ok=True)
    output_file = out_root / "stage_c_statements_with_entities.json"

    print("=" * 70)
    print("STAGE C v3: NER only (no label filter)")
    print("All entity types pass through; filter by UMLS in Stage D.")
    print("=" * 70)
    ...
    print(f"\n[3] Initializing recognize_entities (v3: no label filter)...")
    ...
    output_data = {
        "metadata": {
            "stage": "c",
            "version": "v3",
            "description": "NER only, no label filter. All entity types kept. Filter by UMLS in Stage D.",
            "label_filter": False,
    ...
    print("\n" + "=" * 70)
    print("STAGE C v3 COMPLETE (NER only, no label filter)")
    print("=" * 70)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Stage C v3: NER only, no label filter. All entity types pass through.",
    )
    parser.add_argument(
        "--input-dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help=f"Output directory (default: {V3_OUTPUT_DIR})",
    )
    args = parser.parse_args()
    run_stage_c_v3(input_dir=args.input_dir, output_dir=args.output_dir)
