...
LLM-based extraction of factual statements from guideline text (Stage e).
No factuality validation: experts validate later. Output: subject, predicate, object, exception, duration.
Supports multiple factual statements per paragraph. Optimized with batch processing.
"""
...
class ValidationModel:
    """
    LLM-based extraction of factual statements from text.
    Extracts subject, predicate, object, exception, duration per statement.
    A paragraph may yield zero, one, or several statements. Factuality is not assessed—experts validate later.
    ...
    def extract_statements_batch(
        ...
        """
        Extract factual statements from each text (batch). A paragraph may contain multiple statements.
        
        Args:
            texts: List of candidate texts (chunks/paragraphs)
            entities_list: List of entity lists (one per text)
            max_new_tokens: Max tokens (400 allows multiple statements per chunk)
            batch_size: Number of prompts to process at once
        
        Returns:
            List of statement lists: one list per text, each list has 0+ statements with
            subject, predicate, object, exception, duration.
        """
        import torch
    ...
    def validate_statements_batch(
        ...
        """
        Alias for extract_statements_batch for backward compatibility.
        """
    ...
    def _build_prompt(self, text: str, entities: Optional[List[Dict]] = None) -> str:
        """Build prompt for extraction (no validation)."""
...
"""
    ...
    def _parse_extraction_output(self, raw: str) -> List[Dict]:
        """Parse raw LLM output to list of statement dicts (subject, predicate, object, exception, duration)."""
    ...
    def _parse_single_output(self, raw: str) -> Optional[Dict]:
        """Legacy: parse single statement (returns first statement or None)."""
        statements = self._parse_extraction_output(raw)
        return statements[0] if statements else None
    
    def run_inference(self, prompt: str, max_new_tokens: int = 400) -> str:
        """Run single inference on the LLM (legacy method)."""
        import torch
    ...
    def validate_statement(
        ...
        """
        Validate whether text contains a guideline rule (single statement).
        For better performance, use validate_statements_batch().
        
        Args:
            text: Candidate text
            entities: Optional NER entities (weak signal)
            max_new_tokens: Generation token limit
        
        Returns:
            Validated statement dict or None
        """
    ...
    def _get_system_prompt(self) -> str:
        """System prompt for extraction (multiple statements per text)."""
...
If no factual statement, output: []""".strip()
    ...
    def _extract_json_or_null(self, raw: str) -> Optional[str]:
        """Extract JSON or null from raw output (legacy single object)."""
        if raw is None:
            return None
    ...
    def _extract_json_array_or_null(self, raw: str) -> Optional[List]:
        """Extract JSON array (or single object) from raw output. Returns Python list or None."""
        if raw is None:
            return None
        raw_clean = re.sub(r"^\s*```(?:json)?\s*", "", raw.strip(), flags=re.IGNORECASE)
        raw_clean = re.sub(r"\s*```\s*$", "", raw_clean).strip()
        # Try parse whole output first
        try:
            ...
            pass
        # Find first '[' or '{' and extract balanced bracket range
    ...
    def _normalize_nullish_values(self, d: Dict) -> Dict:
        """Convert null-ish strings to actual None."""
    ...
    def _validate_and_clean_single_statement(self, data: Dict) -> Optional[Dict]:
        """Validate and clean one extracted statement (subject, predicate, object, exception, duration)."""
        # Normalize keys: accept old names from prompts (population→subject, action→split or keep as predicate+object)
        subject = data.get("subject") or data.get("population")
        predicate = data.get("predicate")
        obj = data.get("object")
        exception = data.get("exception")
        duration = data.get("duration")
        # If LLM returned "action" (verb + object), try to use as predicate + object
        ...
        if exception and isinstance(exception, str):
            ...
            exception = None
        # Clean duration (drop vague or non-temporal)
